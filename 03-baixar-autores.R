...
base_bruta_distinta <- readr::read_rds("data/base_bruta_final.rds")
...
possibly_get_user_profile <-
  purrr::possibly(academictwitteR::get_user_profile, otherwise = tibble::tibble(erro = "error"))
...
base_user <- readr::read_rds("data/base_user.rds")
...
iterar_get_user <-
  function(base_user,
    ...
    autores_falta_buscar <- autores_buscar[!autores_buscar %in% base_user$id]
...
while (!"Too Many Requests" %in% base_user$value) {
  base_user <- iterar_get_user(base_user, autores_buscar, amostra = 1000) 
  
  base_user |>
    readr::write_rds("data/base_user.rds")
...
autores_user_arrumado |>
  readr::write_rds("data/base_user_final.rds")
